def jsonToOriginal(uigf_data):
    ...
    for each in data:
        if each["gacha_type"] == "100":
            output_100.append([each["item_type"], each["name"], each])
        elif each["gacha_type"] == "200":
            output_200.append([each["item_type"], each["name"], each])
        elif each["gacha_type"] == "301":
            output_301.append([each["item_type"], each["name"], each])
        elif each["gacha_type"] == "400":
            output_400.append([each["item_type"], each["name"], each])
        elif each["gacha_type"] == "302":
            output_302.append([each["item_type"], each["name"], each])
